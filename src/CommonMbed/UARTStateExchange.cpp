...
UARTStateSender::UARTStateSender(PinName tx) : uart(new mbed::BufferedSerial(tx, NC))
{
	uart->set_baud(9600);
	uart->set_format(
		/* bits */ 8,
...
UARTStateReceiver::UARTStateReceiver(PinName rx) : uart(new mbed::UnbufferedSerial(NC, rx))
{
	uart->baud(9600);
	uart->format(
		/* bits */ 8,
